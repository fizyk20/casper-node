...
#######################################
# Runs an integration tests that tries to fast-sync a validator node
...
function main() {
    log "------------------------------------------------------------"
    log "Validator syncing test begins"
    log "------------------------------------------------------------"
    ...
    do_await_some_blocks
    # 7. Start the node in fast-sync mode using hash from 4)
    do_start_new_node "$SYNC_NODE_ID" 'false'
    # 8. Wait until fast-sync node is synchronized.
    do_await_full_synchronization "$SYNC_NODE_ID"
    # 9. Await some more blocks
    do_await_some_blocks '10'
    # 10. Assert that the syncing validator eventually proposes a block.
    assert_node_proposed "$SYNC_NODE_ID"
    ...
    log "------------------------------------------------------------"
    log "Validator syncing test complete"
    log "------------------------------------------------------------"
...
EOF
    ...
    local CHAINSPEC_PATH="$(get_path_to_stage '1')/1_0_0/chainspec.toml"

    # Configure longer eras in the chainspec.
    local SCRIPT=(
        "import toml;"
        "cfg=toml.load('$CHAINSPEC_PATH');"
        "cfg['core']['minimum_era_height']=5000;"
        "cfg['core']['era_duration']=10hours;"
        "toml.dump(cfg, open('$CHAINSPEC_PATH', 'w'));"
    )
    python3 -c "${SCRIPT[*]}"
}
...
function do_start_network() {
    ...
    nctl-start node=3
    nctl-start node=4
    # do not start node 5 - it will be started later and try to sync with the rest
    sleep 10
}

function do_await_genesis_era_to_complete() {
    log_step "awaiting genesis era to complete"
    # this should work even with longer eras, as era 0 should be just the genesis switch block
...
function do_await_some_blocks() {
    local OFFSET=${1:-'6'}
    # Should be enough to await for six blocks.
    log_step "awaiting six blocks…"
    nctl-await-n-blocks offset='$OFFSET' sleep_interval='4.0' timeout='180'
}
